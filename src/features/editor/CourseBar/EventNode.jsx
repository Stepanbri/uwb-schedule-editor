    ...
    return (
        <Card
            ...
            sx={{
                ...
                backgroundColor: (theme) => isEnrolled ? alpha(theme.palette.success.light, 0.15) : 'transparent',
                borderColor: (theme) => isEnrolled ? theme.palette.success.main : theme.palette.divider,
                '&:hover': {
                    borderColor: (theme) => isEnrolled ? theme.palette.success.dark : theme.palette.primary.main,
                    boxShadow: (theme) => `0 0 0 1px ${isEnrolled ? theme.palette.success.dark : theme.palette.primary.main}`,
                }
            }}
        >
                    ...
                    <Tooltip title={currentDisabledTooltipText}>
                        <span> {/* Wrapper for Tooltip on disabled button */}
                            <ToggleButton
                                ...
                                sx={{ height: '30px', minWidth: '90px', ml: 1, textTransform: 'none', fontSize: '0.75rem' }}
                                disabled={!canEnroll && !isEnrolled}
                            >
                                {isEnrolled ? t('labels.enrolled', 'Zapsáno') : t('labels.enroll', 'Zapsat')}
                            </ToggleButton>
                        </span>
                    </Tooltip>
                </Box>
