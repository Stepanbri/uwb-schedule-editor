...
const CourseLoadSummaryDialog = ({ open, onClose, summary }) => {
    ...
    const allItems = useMemo(() => {
        ...
        const overwritten = (summary.overwritten || []).map(item => ({ ...item, status: 'overwritten' }));
        return [...failed, ...added, ...overwritten];
    }, [summary]);

    const getIconForStatus = (status) => {
        switch (status) {
            case 'failed': return <ErrorOutlineIcon color="error" fontSize="small" />;
            case 'added': return <CheckCircleOutlineIcon color="success" fontSize="small" />;
            case 'overwritten': return <LoopIcon color="info" fontSize="small" />;
            default: return null;
        }
    };
        ...
        <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
            ...
            <DialogContent dividers>
                <Typography gutterBottom>
                    {t('Dialogs.summary.overallResult', 'Proces načítání předmětů ze studijního plánu byl dokončen.')}
                </Typography>
                
                {allItems.length > 0 ? (
                    <List dense sx={{ maxHeight: 350, overflow: 'auto', border: '1px solid', borderColor: 'divider', borderRadius: 1, mt: 2 }}>
