...
import {
    ...
    Typography, styled, useTheme, CircularProgress, alpha
} from '@mui/material';
...
function EditorPage() {
    ...
    const {
        isLoadingWorkspace, isWorkspaceInitialized, initializeWorkspace, workspaceYear, workspaceService,
    ...
    useEffect(() => {
        ...
        isProcessingLoginCallback, stagUserTicket, userInfo,
        stagStudentPlanLoader.processLoginSuccessAndOpenIdentityDialog,
    ...
    const globalLoadingMessage = () => {
        if (stagStudentPlanLoader.isLoadingCourseDetails) return t('alerts.fetchingSubjectDetailsCount', { count: stagStudentPlanLoader.overwritePlanDialogState.coursesToProcess?.length || 0 });
        if (stagStudentPlanLoader.isStudentPlanLoadingActive && !stagStudentPlanLoader.isLoadingCourseDetails && !stagStudentPlanLoader.overwritePlanDialogState.open) return t("common.processingStudentPlan");
        if (stagCourseLoader.isProcessingCourse && !stagCourseLoader.overwriteSingleCourseDialog.open) return t("common.processingCourse");
        ...
        if (!isWorkspaceInitialized && location.pathname === '/editor' && !isLoadingWorkspace) return t("common.loadingApp");
        ...
        <Box sx={{ width: { xs: '85vw', sm: LEFT_SIDEBAR_WIDTH_DESKTOP }, height: '100%', display: 'flex', flexDirection: 'column' }} role="presentation">
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 1, borderBottom: 1, borderColor: 'divider' }}>
                <Typography variant="h6" sx={{ ml: 1 }}>
                    {activeMobileTab === 0 ? t('editorPage.mobileDrawer.coursesTitle') : t('editorPage.mobileDrawer.propertiesTitle')}
                </Typography>
            ...
            <Box sx={{ flexGrow: 1, overflowY: 'auto', position: 'relative' }}>
                {activeMobileTab === 0 && (
                    <CourseBar
                        ...
                        isLoading={!isWorkspaceInitialized && location.pathname === '/editor' && !isLoadingWorkspace }
                ...
                {activeMobileTab === 1 && <PropertiesBar onRemoveAllPreferences={confirmAndRemoveAllPreferences} />}
            </Box>
        </Box>
    );
    ...
    return (
        <EditorPageRoot>
            {isMobile ? (
                <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%'}}>
                    ...
                    <ScheduleBoxWrapper ref={scheduleBoxRef} sx={{ height: scheduleBoxWrapperHeight, m:1, mt:0, borderRadius: theme.shape.borderRadius, flexGrow: 1 }}>
                        ...
                        {(!isEffectivelyLoading && activeSchedule instanceof ScheduleClass && isWorkspaceInitialized) && <ScheduleBox schedule={activeSchedule} />}
                        {(!isEffectivelyLoading && !(activeSchedule instanceof ScheduleClass) && isWorkspaceInitialized) &&
                            <Box sx={{p:2, textAlign:'center', height:'100%', display:'flex', alignItems:'center', justifyContent:'center'}}>
                                <Typography color="text.secondary">{t("common.scheduleNotAvailable")}</Typography>
                            </Box>
                        }
                    </ScheduleBoxWrapper>
            ...
            <ConfirmationDialog
                ...
                message={t('Dialogs.studentPlanOverwrite.message', 'Následující akce ovlivní vaši pracovní plochu. Přejete si pokračovat?')}
